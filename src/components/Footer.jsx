...
export const Footer = () => {
  ...
  return (
    <footer className="relative mt-24">
      <div className="relative z-10 container mx-auto px-5 py-20 md:py-12 rounded-2xl border border-[#1B2A49] supports-[backdrop-filter]:bg-[#0C1220]/40 bg-[#0C1220]/72 backdrop-blur-[3px] overflow-hidden transition-colors duration-200 ease-in-out" style={{ willChange: "opacity" }}>
        <div aria-hidden="true" className="absolute inset-0 footer-glow-overlay" />
        <div className="relative z-10">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-12 md:gap-8">
          <div className="space-y-3">
            <div className="flex items-center gap-3">
              <img src={Logo} alt="Optimum Tech logo" className="h-10 w-10 md:h-7 md:w-7 rounded-md" />
              <span className="text-xl md:text-base font-semibold text-[#E6F1FF]">Optimum Tech</span>
            </div>
            <div className="max-w-xs rounded-2xl border border-[#1B2A49] bg-white/5 px-6 py-5">
              <p className="text-lg md:text-sm text-[#E6F1FF] opacity-90">{t("footer.description")}</p>
            </div>
          </div>
          <div>
            <h4 className="text-xl md:text-base text-[#E6F1FF] font-semibold mb-3">{t("footer.menuTitle")}</h4>
                ...
                >
                  {LANG_OPTIONS.map((opt) => (
                    <li key={opt.code}>
                      <button
                        type="button"
                        onClick={() => { setLang(opt.code); setOpenLang(false); }}
                        className={`w-full text-left px-5 py-3 md:px-3 md:py-2 rounded-md text-lg md:text-sm ${lang === opt.code ? "bg-white/10 text-white" : "text-gray-200 hover:bg-white/10"}`}
                        role="menuitem"
        ...
        <div className="relative z-10 mt-10 pt-6 border-t border-[#1B2A49] flex flex-col md:flex-row items-center justify-between gap-3">
          <p className="text-base md:text-xs text-[#E6F1FF] opacity-80">© 2025 Optimum Tech. {t("footer.rights")}</p>
