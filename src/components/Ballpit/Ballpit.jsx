...
class Z extends d {
  constructor(e, t = {}) {
    const i = { ...X, ...t };
    const s = new z();
    const envTex = new p(e, 0.04).fromScene(s).texture;
    const o = new g();
    const r = new Y({ envMap: envTex, ...i.materialParams });
    r.envMapRotation.x = -Math.PI / 2;
    r.vertexColors = true;
    r.transparent = true;
    r.opacity = i.opacity ?? 0.45;
    if (i.texture) {
      r.map = i.texture;
      r.needsUpdate = true;
    } else if (i.textureUrl) {
      const loader = new Q();
      const tex = loader.load(i.textureUrl);
      tex.colorSpace = n;
      r.map = tex;
      r.needsUpdate = true;
    }
...
function createBallpit(e, t = {}) {
  ...
  if (interactive) {
    ...
    e.style.webkitUserSelect = 'none';
      ...
      pointerController = S({
        domElement: e,
        onMove() {
          n.setFromCamera(pointerController.nPosition, i.camera);
  ...
  i.onBeforeRender = e => {
    if (autoBounce && s?.physics) {
      autoPhase += e.delta * autoBounceSpeed;
      const amp = autoBounceAmplitude;
      const xOffset = Math.sin(autoPhase) * amp * 0.8;
      const yBase = Math.abs(Math.cos(autoPhase * 1.2)) * amp + amp * 0.5;
      s.physics.center.set(xOffset, yBase, 0);
      s.config.controlSphere0 = true;
