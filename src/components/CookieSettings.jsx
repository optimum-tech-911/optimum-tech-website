...
export const CookieSettings = () => {
  ...
  const save = () => { setPrefs({ analytics }); setOpen(false); setShowReopen(false); };
  return (
    <>
      {showReopen && (
        <button type="button" className="ot-cookie-reopen" aria-label={t("cookies.settings")} onClick={() => setOpen(true)}>{t("cookies.reopen")}</button>
      )}
      {open && (
        <div className="ot-cookie-modal" role="dialog" aria-modal="true">
          <div className="ot-modal-card ot-animate-in">
            ...
            <div className="ot-modal-actions">
              <button type="button" className="ot-btn ot-btn-primary" onClick={save}>{t("cookies.save")}</button>
              <button type="button" className="ot-btn ot-btn-secondary" onClick={() => setOpen(false)}>{t("cookies.cancel")}</button>
            </div>
          </div>
        </div>
      )}
