        ...
        <div className="ot-cookie-banner ot-animate-in" role="dialog" aria-live="polite">
          <div className="ot-cookie-content">
            ...
            <div className="ot-cookie-actions">
              <button type="button" className="ot-btn ot-btn-primary" onClick={acceptAll}>{t("cookies.acceptAll")}</button>
              <button type="button" className="ot-btn ot-btn-secondary" onClick={rejectAll}>{t("cookies.rejectAll")}</button>
              <button type="button" className="ot-btn ot-btn-tertiary" onClick={() => setOpenSettings(true)}>{t("cookies.settings")}</button>
            </div>
          </div>
        </div>
      )}
        ...
        <div className="ot-cookie-modal" role="dialog" aria-modal="true">
          <div className="ot-modal-card ot-animate-in">
            ...
            <div className="ot-modal-actions">
              <button type="button" className="ot-btn ot-btn-primary" onClick={saveSettings}>{t("cookies.save")}</button>
              <button type="button" className="ot-btn ot-btn-secondary" onClick={() => setOpenSettings(false)}>{t("cookies.cancel")}</button>
            </div>
          </div>
        </div>
      )}
      {canReopen && (
        <button type="button" className="ot-cookie-reopen" aria-label={t("cookies.settings")} onClick={() => setOpenSettings(true)}>{t("cookies.reopen")}</button>
      )}
    </>
  );
};